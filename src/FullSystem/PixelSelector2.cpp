...
PixelSelector::PixelSelector(int w, int h)
{
	...
	for(int i=0;i<w*h;i++) randomPattern[i] = rand() & 0xFF; // rand() returns random number from 0 to 0x7FFF.
...
void PixelSelector::makeHists(const FrameHessian* const fh)
{
	...
	int h32 = h/32;
	thsStep = w32;
	// Below nested for loop determine gradient histogram of 32x32 cell with 49 bins and it's quantile values
	for(int y=0;y<h32;y++)
		for(int x=0;x<w32;x++)
		{
			float* map0 = mapmax0+32*x+32*y*w;
			int* hist0 = gradHist;// + 50*(x+y*w32);
			memset(hist0,0,sizeof(int)*50); // Set all values in hist0 to 0 , WHY this with number "50"??
			...
			for(int j=0;j<32;j++) for(int i=0;i<32;i++)
			{
				...
				if(g>48) g=48;
				hist0[g+1]++; // Histogram value
				hist0[0]++; // Total count
			}

			ths[x+y*w32] = computeHistQuantil(hist0,setting_minGradHistCut) + setting_minGradHistAdd;
		}
	// below nested for loop process each cell
	for(int y=0;y<h32;y++)
		for(int x=0;x<w32;x++)
		{
			float sum=0,num=0;
			if(x>0) // Handling Left neighbor Cells
			{
			...
			if(x<w32-1) // Handling Right neighbor Cells
			{
				if(y>0) 	{num++; 	sum+=ths[x+1+(y-1)*w32];}
				if(y<h32-1) {num++; 	sum+=ths[x+1+(y+1)*w32];}
				num++; sum+=ths[x+1+(y)*w32];
			...
			if(y>0) 	{num++; 	sum+=ths[x+(y-1)*w32];} // Handling Top neighbor Cells
			if(y<h32-1) {num++; 	sum+=ths[x+(y+1)*w32];} // Handling Bottom neighbor Cells
			num++; sum+=ths[x+y*w32];
...
//	else
	{
		...
		// select!, currentPotential maps to `pot` in select function argument
		Eigen::Vector3i n = this->select(fh, map_out,currentPotential, thFactor);

		// sub-select!
		numHave = n[0]+n[1]+n[2]; // sum of original(level 0), level 1, level 2
		quotia = numWant / numHave; // quotia > 1.0: We have less point then we want. 
									// quotia < 1.0: we have many point then we want.
	...
	if(plot)
	{
		...
		for(int y=0; y<h;y++)
			for(int x=0;x<w;x++)
			{
				int i=x+y*w;
				if(map_out[i] == 1)
					img.setPixelCirc(x,y,Vec3b(0,255,0)); // original scale be green
				else if(map_out[i] == 2)
					img.setPixelCirc(x,y,Vec3b(255,0,0)); // level 1 be red
				else if(map_out[i] == 4)
					img.setPixelCirc(x,y,Vec3b(0,0,255)); // level 2 be blue
			}
		...
		float* map_out, int pot, float thFactor)
{
	...
	int n3=0, n2=0, n4=0;
	for(int y4=0;y4<h;y4+=(4*pot)) for(int x4=0;x4<w;x4+=(4*pot)) // from 0 to pot(=3) [0, h]
	{
		int my3 = std::min((4*pot), h-y4);
		int mx3 = std::min((4*pot), w-x4);
		int bestIdx4=-1; float bestVal4=0;
		Vec2f dir4 = directions[randomPattern[n2] & 0xF];
		for(int y3=0;y3<my3;y3+=(2*pot)) for(int x3=0;x3<mx3;x3+=(2*pot)) // Coarse scale [0, my3], where my3 = 12
		{
			int x34 = x3+x4;
			int y34 = y3+y4;
			...
			for(int y2=0;y2<my2;y2+=pot) for(int x2=0;x2<mx2;x2+=pot) // Medium scale [0, my2], where my2 = 6
			{
				int x234 = x2+x34;
				int y234 = y2+y34;
				...
				for(int y1=0;y1<my1;y1+=1) for(int x1=0;x1<mx1;x1+=1) // Fine scale [0, 4]
				{
					assert(x1+x234 < w);
					assert(y1+y234 < h);
					...
					if(xf<4 || xf>=w-5 || yf<4 || yf>h-4) continue; // offset 4 0,1,2,3 or w-1, w-2, w-3, w-4 will pass
					...
					{
						Vec2f ag0d = map0[idx].tail<2>(); // tail<n> return last n columns. => dx, dy of dIp at index `idx`
					...
					if(bestIdx3==-2) continue; // bestIdx2 is selected. they don't need the rest.
					// [v] ag0 not satisfy magnitude threshold. we use next level. +0.25f is for rounding
					...
					// [v] ag1 not satisfy magnitude threshold. we use next level. +0.125f is for rounding
				...
				if(bestIdx2>0)
				{
					map_out[bestIdx2] = 1; // 1 means original scale?
					bestVal3 = 1e10;
					n2++;
				}
			}
			...
			if(bestIdx3>0)
			{
				map_out[bestIdx3] = 2; // 2 means double?
				bestVal4 = 1e10;
				n3++;
			}
		}
		...
		if(bestIdx4>0)
		{
			map_out[bestIdx4] = 4; 	// 4 means quadra? 1->2->2^2
			n4++;
		}
	}
